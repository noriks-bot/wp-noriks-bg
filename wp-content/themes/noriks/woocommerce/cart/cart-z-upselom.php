    ...
    <!-- !!!!!!     COUNTDOWN    TIMER   !!! --!>
                ...
                <div class="wc-reserve-note" id="wc-reserve-note" role="status" aria-live="polite">
                  ...
                  <span class="wc-reserve-note__text">
                    Моля, побързай! Някой току-що поръча един от продуктите в твоята количка.
                    Резервацията е валидна само още <strong id="wc-reserve-timer">10:00</strong> минути.
                  </span>
                </div>
...
$TRIGGER_CATEGORY_SLUGS = array( 'boksarki', 'komplekti-boksarki', '1-broy-boksarki', 'orto-bokserice', 'komplekti-teniski-i-boksarki', 'black-friday' );
...
<div class="upsell-wrap <?php echo $upsell_in_cart ? 'is-added' : ''; ?>"
  ...
  <div class="upsell-card">
    ...
    <div class="upsell-loader" aria-hidden="true">
      ...
      <div class="upsell-loader__text">Обработва се…</div>
    </div>
  </div>
...
(function(){
  function initUpsellAjax(){
    ...
    function syncAddedUI(force){
      ...
      if (btnText) btnText.textContent = isAdded ? 'ДОБАВЕНО' : 'Добави в количката';
    }
    ...
    checkbox.addEventListener('change', async () => {
      ...
      try{
        ...
        if (checkbox.checked) {
          ...
          if (type === 'variable') {
            if (!hasAllRequiredSelections()) {
              alert('Избери всички опции (например размер) преди добавяне.');
              checkbox.checked = false;
              syncAddedUI(false);
              setBusy(false);
              return;
        ...
        if (!removeUrl) {
          alert('Не мога да премахна продукта (не намирам линка за премахване в количката).');
          checkbox.checked = true;
          syncAddedUI(true);
          setBusy(false);
          return;
