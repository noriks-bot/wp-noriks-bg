...
foreach ( $order->get_items() as $item ) {
    ...
    $is_majica = ( strpos($cat_str, 'teniski') !== false || strpos($name, 'teniski') !== false );
    // Boksarice: category has "boxer/bokser/orto" OR SKU has "box" OR name has "bokser/airflow"
    $is_boks = ( strpos($cat_str, 'boxer') !== false || strpos($cat_str, 'boksark') !== false || strpos($cat_str, 'orto') !== false || strpos($sku, 'box') !== false || strpos($name, 'boksark') !== false || strpos($name, 'airflow') !== false );
    // Komplet
    $is_komplet = ( strpos($name, 'komplekt') !== false || strpos($cat_str, 'komplekt') !== false );
...
if ( $upsell_is_majice ) {
    ...
    $upsell_name       = 'Crne Тениски';
    $upsell_qty_prices = array( 1 => 12.99, 3 => 29.99, 6 => 39.99 );
    $upsell_qty_names  = array( 1 => '1x Crna Majica', 3 => '3x Crne Тениски', 6 => '6x Crnih Majica' );
    $upsell_qty_images = array(
        1 => 'https://noriks.com/bg/wp-content/uploads/2025/09/black-1.jpg',
        3 => 'https://noriks.com/bg/wp-content/uploads/2025/09/black-3x.jpg',
        6 => 'https://noriks.com/bg/wp-content/uploads/2026/01/15xcrnamajica.png',
    );
    $upsell_title_text = 'Добави тениски сега – 50% намаление';
} else {
    ...
    $upsell_qty_images = array(
        1 => 'https://noriks.com/bg/wp-content/uploads/2025/11/crne-boksarice-produktna.jpg',
        3 => 'https://noriks.com/bg/wp-content/uploads/2025/11/boksarice_3x_crne.png',
        5 => 'https://noriks.com/bg/wp-content/uploads/2026/01/boksarice_5x_crne.png',
    );
    $upsell_title_text = 'Добави боксерки сега – 50% намаление';
}
...
<div id="order-splash" style="position:fixed;top:0;left:0;width:100%;height:100%;background:#04ac00;z-index:999999;display:flex;flex-direction:column;align-items:center;justify-content:center;transition:opacity 0.6s ease;">
    ...
    <h1 style="color:#fff;font-family:'Roboto',sans-serif;font-size:28px;font-weight:700;margin:0;">Поръчката е получена!</h1>
...
<div class="ty-container">
    ...
    <?php if ( $order->has_status( 'failed' ) ) : ?>
        <div class="ty-success" style="background:#fde8e8;">
            ...
            <h1>Поръчката не бе успешна</h1>
            <p>Банката отказа транзакцията. Опитайте отново.</p>
            <p style="margin-top:16px;">
                <a href="<?php echo esc_url( $order->get_checkout_payment_url() ); ?>" style="display:inline-block;background:#E8450E;color:#fff;padding:12px 32px;text-decoration:none;font-weight:700;">Опитайте отново</a>
            </p>
        </div>
    <?php else : ?>
        ...
        <div class="ty-success">
            ...
            <p>Потвърждението беше изпратено на <?php echo esc_html( $order->get_billing_email() ); ?></p>
            ...
            <div class="ty_upsell_one_wrapper__popup-content">
                ...
                <div class="tyuo_product_section">
                        ...
                        <select class="variation-select" id="ty-variation-select">
                            <?php if ( $upsell_variations ) : ?>
                                ...
                                <option value="<?php echo $v['id']; ?>" <?php selected( strtolower($v['size']), strtolower($customer_size) ); ?>>
                                    Черно, <?php echo esc_html( $v['size'] ); ?>
                                </option>
                                <?php endforeach; ?>
                            <?php else : ?>
                                <option value="">Черно, S</option>
                                <option value="">Черно, M</option>
                                <option value="">Черно, L</option>
                                <option value="">Черно, XL</option>
                                <option value="">Черно, XXL</option>
                            <?php endif; ?>
                        </select>
                    </div>
                    ...
                    <div class="buttons-section">
                        <a class="pass-btn" id="ty-btn-skip">Не желая</a>
                        <div class="buy-btn" id="ty-btn-add" data-product-id="<?php echo esc_attr( $upsell_product_id ); ?>">ДОБАВИ КЪМ ПОРЪЧКАТА</div>
                    </div>
                </div>
            ...
            <div class="ty-grid-popup">
                <div class="ty-grid-header" style="cursor:pointer;flex-direction:column;position:relative;">
                    ...
                    <div style="display:flex;align-items:center;justify-content:center;gap:10px;margin-bottom:8px;">
                        <div style="font-size:15px;font-weight:400;color:#000;">Специалната оферта изтича след</div>
                    ...
                    <div style="font-size:20px;font-weight:700;color:#000;line-height:1.3;text-align:center;padding-top:10px;">Добавете любов продукт с 50% намаление</div>
                </div>
                <div class="ty-section-body open" id="ty-grid-body">
                <div class="ty-section-body-inner" style="padding:0;">
                <div class="ty-grid-trust">
                    ✔ Всичко изпращаме в един пакет
                </div>
                <div class="ty-grid">
                    ...
                    <div class="ty-grid-item">
                        ...
                        <div class="g-category">БОКСЕРКИ</div>
                ...
                <div class="buttons-section">
                    <a class="pass-btn" id="ty-grid-close">Не желая</a>
                    <div class="buy-btn" id="ty-grid-add-all">ДОБАВИ КЪМ ПОРЪЧКАТА</div>
                </div>
        ...
        <div class="ty-section" id="ty-order-items-section">
            <div class="ty-section-header open" onclick="tyToggle(this)">
                <span id="ty-order-items-header">Артикули в поръчката (<?php echo $order->get_item_count(); ?>)</span>
        ...
        <div class="ty-section">
            ...
            <div class="ty-section-body open">
                <div class="ty-section-body-inner">
                    <div class="ty-row"><span class="ty-row-label">Име</span><span class="ty-row-value"><?php echo esc_html( $order->get_billing_first_name() . ' ' . $order->get_billing_last_name() ); ?></span></div>
                    <div class="ty-row"><span class="ty-row-label">Адрес</span><span class="ty-row-value"><?php echo esc_html( $order->get_billing_address_1() . ' ' . $order->get_billing_address_2() ); ?></span></div>
                    <div class="ty-row"><span class="ty-row-label">Град</span><span class="ty-row-value"><?php echo esc_html( $order->get_billing_postcode() . ' ' . $order->get_billing_city() ); ?></span></div>
                    <?php if ( $order->get_billing_phone() ) : ?>
                    <div class="ty-row"><span class="ty-row-label">Телефон</span><span class="ty-row-value"><?php echo esc_html( $order->get_billing_phone() ); ?></span></div>
                    <?php endif; ?>
...
<?php else : ?>
    <div class="ty-container">
        <div class="ty-success"><h1>Поръчка</h1>
...
(function(){
    ...
    function tick() {
        if (rem <= 0) {
            ...
            // Auto-open "Артикули в поръчката" and "Адрес за доставка" sections
        ...
        fetch(ajaxUrl, { method: 'POST', body: rfd })
            ...
            .then(function(d) {
                if (d.success) {
                    ...
                    if (headerSpan) {
                        headerSpan.textContent = 'Артикули в поръчката (' + d.data.item_count + ')';
                    }
        ...
        addBtn.addEventListener('click', function() {
            ...
            fetch(ajaxUrl, { method: 'POST', body: fd })
                ...
                .catch(function() {
                    ...
                    addBtn.textContent = 'ДОБАВИ КЪМ ПОРЪЧКАТА';
                });
        });
    }
    ...
    if (overlay) {
        ...
        if (gridAddAll) {
            gridAddAll.addEventListener('click', function() {
                ...
                gridAddAll.textContent = 'Добавям...';
                ...
                fetch('<?php echo admin_url("admin-ajax.php"); ?>', { method: 'POST', body: rfd })
                    ...
                    .then(function(d2) {
                        if (d2.success) {
                            ...
                            var headerSpan = document.getElementById('ty-order-items-header');
                            if (headerSpan) {
                                headerSpan.textContent = 'Артикули в поръчката (' + d2.data.item_count + ')';
                            }
                        }
                    });
            } else {
