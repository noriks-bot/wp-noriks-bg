    ...
    <!-- !!!!!!     COUNTDOWN    TIMER   !!! --!>
                ...
                <div class="wc-reserve-note" id="wc-reserve-note" role="status" aria-live="polite">
                  ...
                  <span class="wc-reserve-note__text">
                    Моля, побързайте! Някой току-що поръча един от продуктите във вашата кошница. 
                    Резервацията е валидна за <strong id="wc-reserve-timer">10:00</strong> минути.
                  </span>
                </div>
...
<div class="upsell-wrap <?php echo $upsell_in_cart ? 'is-added' : ''; ?>"
  ...
  <div class="upsell-card">
      ...
      <div class="gck-popular-badge">Специална оферта <img draggable="false" role="img" class="emoji" alt="🔥" src="https://s.w.org/images/core/emoji/17.0.2/svg/1f525.svg"></div>
      ...
      <div class="upsell-body">
        ...
        <div class="upsell-actions">
          <button type="button" class="upsell-add-btn" id="upsell-add-btn">
            <span id="upsell-btn-text"><?php echo $upsell_in_cart ? 'ДОБАВЕНО' : 'Добави в кошницата'; ?></span>
          </button>
...
(function(){
  function initUpsellAjax(){
    ...
    function syncAddedUI(force){
      ...
      if (btnText) btnText.textContent = isAdded ? 'ДОБАВЕНО' : 'Добави в кошницата';
    }
      ...
      try{
        ...
        if (checkbox.checked) {
          ...
          if (!res.ok || data?.error) {
            checkbox.checked = false;
            syncAddedUI(false);
            alert('Не мога да добавя продукта. Проверете избраните опции (вариации) и наличността.');
            setBusy(false);
            return;
          }
        ...
        if (!removeUrl) {
          alert('Не мога да премахна продукта (не мога да намеря линка за премахване в кошницата).');
          checkbox.checked = true;
          syncAddedUI(true);
          setBusy(false);
          return;
